...
const defaultLinks: NavbarLink[] = [
  ...
  { label: 'Popular Places', to: '/popular' },
        ...
        <Toolbar disableGutters sx={{ gap: 2 }}>
          ...
          <Stack direction="row" spacing={5} sx={{ display: { xs: 'none', md: 'flex' }, mx: 'auto' }}>
            {links.map((l) => (
              <Typography
                ...
                sx={{ color: isActive(l.to) ? '#FF8C75' : '#111827', fontWeight: 700 }}
              >
                {l.label}
              </Typography>
            ))}
          ...
          {showAuth && (
            <Stack direction="row" spacing={2} sx={{ display: { xs: 'none', md: 'flex' }, ml: 'auto' }} alignItems="center">
              <Typography component={Link} to="/auth/login" sx={{ textDecoration: 'none', color: '#111827', fontWeight: 700 }}>Log in</Typography>
              <Button component={Link} to="/auth/signup" variant="contained" sx={{ bgcolor: '#FF8C75', borderRadius: 999, px: 3, py: 1.25, textTransform: 'none', fontWeight: 700, '&:hover': { bgcolor: '#ff7a61' } }}>
                Sign up
              </Button>
            </Stack>
          )}
        ...
        <Box sx={{ position: 'fixed', inset: 0, bgcolor: 'rgba(0,0,0,0.4)', zIndex: 1200 }} onClick={() => setOpen(false)}>
          <Box sx={{ position: 'absolute', top: 0, right: 0, width: '80%', maxWidth: 320, height: '100%', bgcolor: '#fff', p: 3 }} onClick={(e) => e.stopPropagation()}>
            ...
            <Stack spacing={2.5} sx={{ mb: 3 }}>
              {links.map((l) => (
                <Typography key={l.label} component={Link} to={l.to} onClick={() => setOpen(false)} sx={{ textDecoration: 'none', color: isActive(l.to) ? '#FF8C75' : '#111827', fontWeight: 700 }}>
                  {l.label}
                </Typography>
              ))}
            </Stack>
            {showAuth && (
              <Stack direction="row" spacing={1.5}>
                ...
                <Button component={Link} to="/auth/signup" variant="contained" onClick={() => setOpen(false)} sx={{ bgcolor: '#FF8C75', borderRadius: 999, px: 3, textTransform: 'none', fontWeight: 700, '&:hover': { bgcolor: '#ff7a61' } }}>Sign up</Button>
              </Stack>
            )}
          </Box>
        </Box>
