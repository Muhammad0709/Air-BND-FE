            ...
            <CardContent>
              <Stack direction={{ xs: 'column', sm: 'row' }} alignItems={{ xs: 'flex-start', sm: 'center' }} justifyContent="space-between" sx={{ mb: 3, gap: 2 }}>
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems="center" sx={{ width: { xs: '100%', sm: 'auto' } }}>
                  ...
                  <Button
                    variant="contained"
                    onClick={() => navigate('/host/bookings/create')}
                  ...
                  <TableBody>
                    {filteredBookings.map((booking) => (
                      <TableRow key={booking.id} sx={{ '&:hover': { bgcolor: '#F9FAFB' } }}>
                        <TableCell sx={{ fontWeight: 600, color: '#111827' }}>{booking.guest}</TableCell>
                        <TableCell sx={{ color: '#6B7280' }}>{booking.property}</TableCell>
                        <TableCell sx={{ color: '#6B7280' }}>{booking.checkin}</TableCell>
                        <TableCell sx={{ color: '#6B7280' }}>{booking.checkout}</TableCell>
                        <TableCell>
                          <Chip
                        ...
                        <TableCell>
                          <ActionsMenu
                            onView={() => navigate(`/host/bookings/show/${booking.id}`)}
                            onEdit={() => navigate(`/host/bookings/edit/${booking.id}`)}
